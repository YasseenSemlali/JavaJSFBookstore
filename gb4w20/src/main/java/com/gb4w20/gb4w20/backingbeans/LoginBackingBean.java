...
public class LoginBackingBean implements Serializable {
    ...
    /**
     * Used to login based on the values given. If the login is successful there
     * is two possibilities: 1) If a manager redirected to to the manager page
     * 2) If a normal client redirect to the index.xhtml page If login failed
     * prompted with error
     *
    ...
    public void login() throws IOException {
        ...
        if (this.session.loginUser(this.emailInput, this.passwordInput)) {
            ...
            context.redirect(context.getRequestContextPath() + "/" + (this.session.isLoggedInManager() ? MANAGER_PAGE : CLIENT_PAGE));
        }
    ...
    /**
     * Used to validate if the user entered a value. Design decision to only
